...
int main (int argc, char *argv[]) {
    ...
    FILE *serverFp;
    serverFp = fopen("server_pubkey.pub", "r");
    RSA *serverPubKey = PEM_read_RSAPublicKey(serverFp, NULL, NULL, NULL);
    fclose(serverFp);
    ...
    for (int i=0; i<=10000; i++) {
        ...
        if (BN_cmp(rsa->n, serverPubKey->n) == 0) {
           printf("seed = %d\n", i);       
